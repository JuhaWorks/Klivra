...
const TimelineView = ({ tasks, onOpenTask }) => {
    ...
    const roadmapTasks = useMemo(() => {
        return tasks.filter(t => t.dueDate).sort((a,b) => new Date(a.startDate || a.dueDate) - new Date(b.startDate || b.dueDate));
    }, [tasks]);
        ...
        <div className="bg-sunken/20 border border-white/[0.03] rounded-[3rem] p-8 shadow-2xl backdrop-blur-xl overflow-hidden flex flex-col">
            <header className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button 
                        onClick={centerView}
                        className="flex items-center gap-2 px-4 py-2 bg-theme/10 border border-theme/20 rounded-xl text-[10px] font-black text-theme uppercase tracking-widest hover:bg-theme hover:text-white transition-all"
                    >
                            ...
                            return (
                                <motion.div 
                                    ...
                                    style={{ 
                                        ...
                                        width: isMilestone ? 40 : Math.max(dayWidth * 0.5, (duration || 0) * dayWidth),
                                        backgroundColor: isMilestone ? 'transparent' : priorityColor,
                                        borderColor: isMilestone ? 'transparent' : priorityColor.replace('0.4', '0.6')
                                    }}
                                    onClick={() => onOpenTask(task)}
                                    className={twMerge(clsx(
                                        "relative h-14 transition-all flex flex-col justify-center",
                                        !isMilestone ? "rounded-2xl border-2 p-3 group cursor-pointer hover:scale-[1.02] hover:z-20 shadow-2xl backdrop-blur-md overflow-hidden" 
                                ...
                                >
                                    {!isMilestone ? (
                                        <>
                                            ...
                                            <div className="flex items-center gap-3 relative z-10">
                                                <div className="w-8 h-8 rounded-lg bg-black/40 flex items-center justify-center shrink-0 border border-white/10 group-hover:border-theme/40 transition-all">
                                                    {(task.assignees?.[0] || task.assignee) ? (
                                                        <img src={getOptimizedAvatar((task.assignees?.[0] || task.assignee).avatar, 'xs')} className="w-full h-full object-cover" alt="avatar" />
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-[7px] font-black text-theme uppercase tracking-widest font-mono">{Math.round(duration || 0)} Days</span>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex flex-col items-center">
                                            <div className="w-4 h-4 rounded-sm rotate-45 bg-theme border-2 border-white/20 group-hover:scale-125 transition-transform shadow-[0_0_15px_rgba(var(--theme-rgb),0.5)]" />
                                            <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-black/80 px-2 py-1 rounded-lg border border-white/10 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity z-30 pointer-events-none">
                                                <p className="text-[8px] font-black text-white uppercase tracking-widest">{task.title}</p>
                                            </div>
                                        </div>
                                    )}
                                </motion.div>
